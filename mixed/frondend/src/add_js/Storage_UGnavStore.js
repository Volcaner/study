...
const state = {
	...
	s2:{
		list:[],//{imgurl,title,sku,startTime,amount,distance,isopen,timer,showtime,groupId,isshow,detailsurl,tuanurl}
	},
	s3:{
		list:[],//{imgurl,title,sku,startTime,endTime,succtype,status,distance,timer,showtime,groupId,adjusturl,detailsurl,isshow}
	},
	s4:{
		list:[],//{imgurl,title,sku,bname,wangwangurl,wangwangimg,startTime,endTime,overtype,status,createTime,detailsurl,isshow}
	},
}
const mutations = {		
	page(state,n){
	...
	s1t1(state,response){
		state.s1.num.left=response.body.wCount;
		state.s1.num.right=response.body.rCount;
		state.allpage=response.body.pages;
		...
		for(let key in list){
			...
			/*let bname=list[key].name;*/
			let bname=(list[key].business && list[key].business.name)?list[key].business.name:'';
			...
			let detailsurl='/yich/Storage/Storage_details.html?groupId='+list[key].groupId;
	...
	s2(state,response){
		...
		for(let key in list){
			...
			let distance=parseInt(604800-list[key].endTime)*1000||0;//结束时间的毫秒数
			...
			let isshow=true;
			let homeurl = '/yich/GoodInfo?proid=' + list[key].proId;
			let detailsurl='/yich/Storage/Storage_fightGroupDoing.html?groupId='+list[key].groupId;
			let tuanurl='/yich/SpellDetailsPage?groupId='+list[key].groupId;
			state.s2.list.push({homeurl:homeurl,imgurl:imgurl,title:title,sku:sku,startTime:startTime,amount:amount,distance:distance,isopen:isopen,timer:timer,showtime:showtime,groupId:groupId,isshow:isshow,detailsurl:detailsurl,tuanurl:tuanurl})
		}
	...
	s3(state,response){
		...
		for(let key in list){
			...
			succtype=succtype=='0'?'拼团失败':succtype=='1'?'拼团成功':succtype=='2'?'提前接单':succtype=='3'?'待确定':succtype=='4'?'到期未拼成':'未知';
			let status=list[key].status;//3为待调剂，4为调剂结束，其他不管
			let distance=parseInt(172800-list[key].orderTime)*1000||0;//生成订单时间的毫秒数
			let timer=null;
			let showtime='';
			let groupId=list[key].groupId;
			let isshow=true;
			let homeurl = '/yich/GoodInfo?proid=' + list[key].proId;
			let adjusturl='/yich/Storage/Storage_Swap.html?groupId='+list[key].groupId;
			let detailsurl='/yich/Storage/Storage_fightGroupAdmin.html?groupId='+list[key].groupId;
			state.s3.list.push({homeurl:homeurl,imgurl:imgurl,title:title,sku:sku,startTime:startTime,endTime:endTime,succtype:succtype,status:status,distance:distance,timer:timer,showtime:showtime,groupId:groupId,isshow:isshow,adjusturl:adjusturl,detailsurl:detailsurl});
		}
			...
			(function(list){
				...
				list.timer=setInterval(function(){
					if(isNaN(list.distance)||list.distance<=0){
						...
						Vue.http.post('/yich/GroupGenerateOrderServlet',{option:'2',groupId:groupId,status:status})
						.then((response)=>{
							window.checkErrorVue(response);
	...
	s4(state,response){
		...
		var list=response.body.list;
		for(let key in list){
			//{imgurl,title,sku,bname,wangwangurl,wangwangimg,startTime,endTime,overtype,status,createTime}
			let imgurl=window.imgchange(list[key].mainSrc,'@80w_80h');
			let title=list[key].proName;
			let sku=list[key].goodNo;
			let bname=list[key].name;
			...
			let homeurl = '/yich/GoodInfo?proid=' + list[key].proId;
			let detailsurl='/yich/Storage/Storage_fightGroupNotes.html?groupId='+list[key].groupId;
			let isshow=true;
			state.s4.list.push({homeurl:homeurl,imgurl:imgurl,title:title,sku:sku,bname:bname,wangwangurl:wangwangurl,wangwangimg:wangwangimg,startTime:startTime,endTime:endTime,overtype:overtype,status:status,createTime:createTime,detailsurl:detailsurl,isshow:isshow})
		}
	},
}
const actions={
	Ajax(context){
		if(context.state.state=='s1'){
			if(context.state.s1.state=='t1'){
				Vue.http.post('/yich/AuditingRecordServlet',{page:context.state.nowpage,status:'wait'})
				.then((response)=>{
					window.checkErrorVue(response);
				...
				Vue.http.post('/yich/AuditingRecordServlet',{page:context.state.nowpage,status:'result'})
				.then((response)=>{
					window.checkErrorVue(response);
			...
			Vue.http.post('/yich/GroupingServlet',{page:context.state.nowpage})
			.then((response)=>{
				window.checkErrorVue(response);
			...
			Vue.http.post('/yich/GroupManageServlet',{page:context.state.nowpage})
			.then((response)=>{
				window.checkErrorVue(response);
			...
			Vue.http.post('/yich/GroupedRecordServlet',{page:context.state.nowpage})
			.then((response)=>{
				window.checkErrorVue(response);
