...
const mutations = {		
	...
	pubAjax(storestate,_list){
		...
		for(var i=0;i<_list.trade_list.length;i++){
			...
			let shopname=trade.sup_name?trade.sup_name:'';
			...
			if(state=='wfk'){
				...
				(function(time,index){
					...
					if(time<0){
						//先判断是否有alipay_num，有就先payoff再delete，没有直接delete
						if(storestate.list.trade[index].bigmsg.alipay_num && storestate.list.trade[index].bigmsg.alipay_num !='N'){
							Vue.http.post('/yich/PayOff',{payNum:state.data[key].dealnumber})
							.then((response)=>{
								window.checkErrorVue(response);
								if(response.body.flag=='1'){
									Vue.http.post("/yich/DeleteOrderServlet",{tra_id:storestate.list.trade[index].bigmsg.ddnumber,option:0,trade_flag:'norecly'})
									.then((response)=>{
										window.checkErrorVue(response);
										if(response.body.exceptionState==1){
											alert('异常')
										}else{
					...
					storestate.wfkstate.timer[index]=setInterval(function(){										
						...
						if(time<0){
							...
							//先判断是否有alipay_num，有就先payoff再delete，没有直接delete
							if(storestate.list.trade[index].bigmsg.alipay_num && storestate.list.trade[index].bigmsg.alipay_num !='N'){
								Vue.http.post('/yich/PayOff',{payNum:storestate.list.trade[index].bigmsg.alipay_num})
								.then((response)=>{
									window.checkErrorVue(response);
									if(response.body.flag=='1'){
										Vue.http.post("/yich/DeleteOrderServlet",{tra_id:storestate.list.trade[index].bigmsg.ddnumber,option:0,trade_flag:'norecly'})
			...
			for(var j=0;j<trade.tradeOrderList.length;j++){
				...
				if(nowstate=='wfk'){//未付款
					...
					if(state=="N"){
						...
						ccurl='/yich/RefundedToPage?'+arb_id+'status=business&tk=1&proId='+ strade.shopInv.pro_id+'&orderId='+strade.orderId+'&type=t';//type:判断跳转仓储还是商户页面（商户：type任意值）
...
const actions={
	Ajax(context){
		...
		s=s=="sydd"?0:s=='wfk'?1:s=='wfh'?2:s=='yfh'?3:s=='jtkz'?4:s=='thz'?5:s=='hhz'?6:s=='jzz'?7:s=='jzwc'?8:s=='jycg'?9:s=='jygb'?10:s=="pt"?'-1':s=="zfdd"?'-2':11;  // :s=="zfdd"?14 作废订单
		//多加的回收站判断				
		if(state.pstate.hstate=='back'){
			m='';
			s=state.pstate.dstate=='sydd'?11:state.pstate.dstate=='jycg'?12:13;
		}
		Vue.http.post('/yich/userMergeOrder',{pno:state.nowpage,month:m,orderNum:state.inputs.sdd,proName:state.inputs.spro,proGoodNo:state.inputs.ssku,business:state.inputs.scname,all_order_status:state.inputs.ssname,allyc_order_status:state.inputs.stel,orderStart:state.inputs.sstart.time,orderEnd:state.inputs.send.time,state:s,tbid:state.inputs.tbid},{emulateJSON:true})
		.then((response)=>{
			window.checkErrorVue(response);
