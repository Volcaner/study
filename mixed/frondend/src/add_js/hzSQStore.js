...
const state={
	haveStarData:'',//已申请数据
	listStarData:'',//申请记录数据
	searchSta:'ing',//查询状态{已申请ing&申请记录A}
	searchName:'',//查询名字
...
const mutations={
	...
	//已申请ajax
	haveStarAjax(state){
		Vue.http.post('/yich/uApplyed?page='+state.page+'&order='+state.sortStaP).then((response)=>{
			//success
	...
	//申请记录ajax & 排序
	listStarAjax(state){
		Vue.http.post('/yich/applyedAndHandleRecord?option=U&status='+state.applySta+'&page=1&order='+state.sortSta).then((response)=>{
			window.checkErrorVue(response);
			//success
			 window.checkErrorVue(response);
			 if(response.data.check){
	...
	//查询
	searchAjax(state){
		Vue.http.post('/yich/queryCoop?option=U&page=1&status='+state.searchSta+'&order='+state.sortSta+'&name='+state.searchName).then((response)=>{
			//success
			 window.checkErrorVue(response);
			if(response.data.list!="null"){
...
function haveData(data){
	var list = [];
	for(var i in data){
		var json={name:'',hzFrends:0,tkPercent:0,rzTime:'',aplyTime:'',aplyKinds:'新的申请',aplySta:'已申请',dengj:'C',dengjX:'1',supshopId:'',wangwang:''};
		...
		json.dengj = data[i].sup.level.split('-')[0];
		json.dengjX = parseInt(data[i].sup.level.split('-')[1]);
