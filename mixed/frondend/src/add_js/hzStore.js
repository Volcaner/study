...
const mutations={
	//单日初始请求数据
	dayStarAjax(state){
		Vue.http.post('/yich/dayTrade?option=sup&date='+state.timeData+'&supShopId='+state.supShopId+'&userId='+state.userId).then((response) => {
				// 响应成功回调
			 window.checkErrorVue(response);
				...
				if(response.data.updateDynamics == 1){
					state.sxStatus = true;
				}else if(response.data.updateDynamics == 0){
					state.sxStatus = false;
				}
			}, (response) => {
				// 响应错误回调
		});
	...
	//累计交易初始请求数据
	allStarAjax(state){
		Vue.http.post('/yich/coopRelation?option=sup&supShopId='+state.supShopId+'&userId='+state.userId+'&coopId='+state.coopId).then((response) => {
				// 响应成功回调
			 window.checkErrorVue(response);
				if(response.data){
	...
	upNewGoods(state){
		var sta = '';
		if(state.sxStatus){sta=0}else{sta=1};
		Vue.http.post('/yich/CloseUpdateInfoServlet',{supShopId:state.supShopId,updateDynamics:sta},{emulateJSON:true}).then((response) => {
				// 响应成功回调
			window.checkErrorVue(response);
			 var data = typeof(response.data)=='string'?JSON.parse(response.data):response.data;
			if(data.result == 1){
				state.sxStatus = !state.sxStatus;
			}
			}, (response) => {
				// 响应错误回调
		});
...
function relationData(data){
	var json={name:'',agreeTime:'',sentKinds:'0',clNum:'0',priceTemp:'0',pfScore:'0',totalMoney:'',totalNum:'',userId:''}
	if(data.c){
		json.name = data.c.supName;
		...
		json.userId = data.c.userId;
	}
	state.relationData = json;
}
function setdafen(data){
	var fenshu = {priceTd:0,goodsTd:0,dateTd:0,serveTd:0,xinyuTd:0,peiheTd:0};
	fenshu.priceTd = parseInt(data.priceSatisfiedScore);
	fenshu.goodsTd = parseInt(data.qualitySatisfiedScore);
	fenshu.dateTd = parseInt(data.deliverySatisfiedScore);
	fenshu.serveTd = parseInt(data.serviceSatisfiedScore);
	fenshu.xinyuTd = parseInt(data.supplyCreditScore);
	fenshu.peiheTd = parseInt(data.supplierCoopScore);
