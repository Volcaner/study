...
const mutations={
	dataStarAjax(state){
		Vue.http.post('/yich/traData',{option:"sup"},{emulateJSON:true}).then((response) => {
			// 响应成功回调
			...
			//近30天商品
			if(response.data.goodMap){
				threeMonthData(response.data.goodMap);
			}
		},(response) => {
			// 响应错误回调
		});
...
//30天商品分析
function threeMonthData(data){
	var traArray=[];
	var retArray=[];
	
	if(data.tragood){
		var traData = data.tragood;
		for(var i in traData){
			var jsonTra={imgSrc:'',goodsName:'',goodsNum:'',proId:''};
			jsonTra.imgSrc = imgchange((traData[i].proImage.src),"@50w_50h");
			jsonTra.goodsName = traData[i].pro_name;
			jsonTra.goodsNum = traData[i].month_tra_sale;
			jsonTra.proId = traData[i].pro_id;
			traArray.push(jsonTra);
		}
		
	};
	if(data.retgood){
		var retData = data.retgood;
		for(var i in retData){
			var jsonRet={retImgSrc:'',retGoodsName:'',retGoodsNum:'',proId:''};
			jsonRet.retImgSrc = imgchange((retData[i].proImage.src),"@50w_50h");
			jsonRet.retGoodsName = retData[i].pro_name;
			jsonRet.retGoodsNum = retData[i].month_ret_num;
			jsonRet.proId = retData[i].pro_id;
			retArray.push(jsonRet);
		}
	}
	
	state.tragood = traArray;
	state.retgood = retArray;
}
//注：勿动（小心弄乱）
