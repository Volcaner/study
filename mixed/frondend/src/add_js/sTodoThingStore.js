...
const mutations = {
		...
		starAjaxData(state){
			Vue.http.post('/yich/SupThingList',{page:state.page,status:'a',type:state.sta},{emulateJSON:true}).then((response) => {
...
const actions={
		...
		delAjax(context){
			Vue.http.post('/yich/SupDeleteThing',{thingId:state.thingId},{emulateJSON:true}).then((response) => {
				window.checkErrorVue(response);
